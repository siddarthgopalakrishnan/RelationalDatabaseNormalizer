	...
	public static ArrayList<Relation> decomposeInto3NFScheme(Relation relation){
		...
		for(int i=0; i<minimalCover.size(); i++){
			...
			Utils.sortAttributes(f.getRightSideAttributes());
                        f.computeNormalForm(relation.getKeyAttributes(), relation.getNonKeyAttributes(), relation.getCandidateKeys());
		}
		...
		if(!relationAttributes.isEmpty()){
			Iterator<FunctionalDependency> itr1 = minimalCover.iterator();
				while(itr1.hasNext()){
					FunctionalDependency f = itr1.next();
//					 if(!relationAttributes.containsAll(f.getLeftSideAttributes()) && !relationAttributes.containsAll(f.getRightSideAttributes())){
////					 	itr.remove();
////                                                relationAttributes.addAll(f.getLeftSideAttributes());
////                                                relationAttributes.addAll(f.getRightSideAttributes());
//					 }
		...
		System.out.println(threeNFRelations);
